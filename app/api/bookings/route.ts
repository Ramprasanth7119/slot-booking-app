...
export async function POST(request: Request) {
  try {
    ...
    // Attempt atomic capacity increment
    const updated = await db.collection<SlotDocument>(SLOTS).findOneAndUpdate(
    ...
    const inserted = { ...bookingDoc, _id: res.insertedId } as BookingDocument;

    return NextResponse.json({ booking: serializeBooking(inserted) }, { status: 201 });
  } catch (error) {
